...
public class HomescreenActivity extends EasyAccessFragment implements KeyListener{
	/**
	...
	*/
	...
	/**
	* Attaches onClick, onFocus and Key listener to the button passed as a paramater, and invokes 
	* startNewActivity method when the button is clicked. The startNewActivity method takes as 
	* parameter, the class of the activity to be launched. When the button receives focus, 
	* giveFeedback method is invoked, that reads aloud the text on the button.
	* @param button The button with which the listeners are to be associated.
	* @param className The class of the activity to be launched.
	*/
	void attachListener(final Button button, @SuppressWarnings("rawtypes") final Class className) {
	...
	*/
	...
	public boolean onKeyDown(View view, Editable text, int keyCode,
		...
		case KeyEvent.KEYCODE_DEL://go to the previous screen
        	...
        	if(!Utils.isAccessibilityEnabled(getActivity().getApplicationContext()) && getResources().getConfiguration().keyboard != Configuration.KEYBOARD_NOKEYS)
        		TTS.speak("Back");
        	getActivity().finish();
        	break;
        case KeyEvent.KEYCODE_F1://go to the home screen
        	...
        	if(!Utils.isAccessibilityEnabled(getActivity().getApplicationContext()) && getResources().getConfiguration().keyboard != Configuration.KEYBOARD_NOKEYS)
        		TTS.speak("Home");
			getActivity().finish();
			Intent intent = new Intent(getActivity().getApplicationContext(), SwipingUtils.class);
