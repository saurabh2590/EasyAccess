...
/**
* The Call Log App activity is used for listing out the outgoing, incoming and missed calls and the 
* total duration of the calls that have been received and dialed.
*/
...
public class CallLogApp extends EasyAccessActivity {
	...
	private Button tabCallsDialled, tabCallsReceived, tabCallsMissed, tabCallsAll, tabTime;
	...
	/**
	* Launches the CallLogOptions activity and passes the name, number and id of the contact.
	* @param position The position of the item that was selected in the ListView.
	*/
	void startNewActivity(int position) {
		if(CallLogApp.this.records.size() != 0) {
			Intent intent = new Intent(getApplicationContext(), CallLogOptions.class);
			intent.putExtra("name", ((HashMap<String, String>)(CallLogApp.this.records.get(position))).
					get("name").toString());
			intent.putExtra("number", ((HashMap<String, String>)(CallLogApp.this.records.get(position))).
					get("number").toString());
			intent.putExtra("id", ((HashMap<String, String>)(CallLogApp.this.records.get(position))).
					get("id").toString());
	...
	* @param typeOfLog Determines whether incoming or outgoing or all the logs are to be displayed.
	**/
	...
	**/
	void getCallLogs(int type) {
		...
		switch(type) {
		case TYPE_DIALED:
			cursor = getContentResolver().query(Uri.parse("content://call_log/calls"), null, 
		...
		case TYPE_ALL:
			...
			break;
		case TYPE_TIME:
			cursor = getContentResolver().query(Uri.parse("content://call_log/calls"), 
					new String[] {CallLog.Calls.DURATION}, "type = " + CallLog.Calls.OUTGOING_TYPE,
					null, null);
			long totalOutgoingTime = 0, totalIncomingTime = 0;
			while(cursor.moveToNext()) {
				totalOutgoingTime += cursor.getLong(cursor.getColumnIndex(CallLog.Calls.DURATION));
			}
			cursor = getContentResolver().query(Uri.parse("content://call_log/calls"), new String[] 
					{CallLog.Calls.DURATION}, "type = " + CallLog.Calls.INCOMING_TYPE, null, null);
			while(cursor.moveToNext()) {
				totalIncomingTime += cursor.getLong(cursor.getColumnIndex(CallLog.Calls.DURATION));
			}
		...
		if(cursor != null && cursor.getCount() == 0) {
			...
			while(cursor.moveToNext()) {
				...
				if(cursor.getString(cursor.getColumnIndex(CallLog.Calls.CACHED_NAME)) != null)
					values.putString("name", cursor.getString(cursor.getColumnIndex(CallLog.Calls.
							CACHED_NAME)) + " ");
				else
				...
				if(cursor.getString(cursor.getColumnIndex(CallLog.Calls.CACHED_NUMBER_LABEL)) != null)
					values.putString("label",cursor.getString(cursor.getColumnIndex(CallLog.Calls.
							CACHED_NUMBER_LABEL)) + Html.fromHtml("<br/>"));
				else
				...
				if(Integer.toString(cursor.getInt(cursor.getColumnIndex(CallLog.Calls._ID))) != null)
					values.putString("id", cursor.getString(cursor.getColumnIndex(CallLog.Calls._ID)));
				else
					values.putString("id", "");
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		topBar = new String[] {getResources().getString(R.string.tabCallsDialled), 
		...
		/** If Dialed tab is pressed, display dialed calls; clicking on the contact name would dial
		 *  the number **/
		...
		/** If Received tab is pressed, display received calls; clicking on the contact name would 
		 * dial the number **/
		...
		/** If Missed tab is pressed, display missed calls; clicking on the contact name would 
		 * dial the number **/
		...
		/** If All tab is pressed, display all calls; clicking on the contact name would 
		 * dial the number **/
		...
		/** If Total Time tab is pressed, display the total incoming and outgoing minutes **/
				...
				if(keyEvent.getAction() == KeyEvent.ACTION_DOWN) {
					...
					case KeyEvent.KEYCODE_DPAD_DOWN:
						...
						if(currentSelection == lstView.getCount()) {
							...
							giveFeedback(callLogsListView.getItemAtPosition(currentSelection).toString());
			...
			public void handleMessage(Message message) {
				...
				if(bundle.size() <= 1) {
					...
					Toast.makeText(getApplicationContext(), 
							getResources().getString(R.string.emptylog), Toast.LENGTH_SHORT).show();
				}
				else {
						...
						bundle.getLong("outtime") + " seconds");
							...
							for(int i=0; i<bundle.size() - 1; i++) { // - 1 for excluding type
								...
								strRecords.add(tempBundle.getString("name") + 
										...
										tempBundle.getString("number") + tempBundle.getString("date"));
				...
				if(bundle.getInt("type") == TYPE_TIME) {
					callLogsAdapter = new CommonAdapter(getApplicationContext(), strRecords, 0);
				}
				else {
					callLogsAdapter = new CommonAdapter(getApplicationContext(), strRecords, 1);
				}
