...
/**
 * Activity that can be used by the user to select or type the name or number of
 * the recipient of the text message.
 **/

public class TextMessagesComposerRecipientApp extends Activity implements
		KeyListener {
	...
	/**
	 * Launches the TextMessagesComposerApp activity and passes the recipient's
	 * number to the activity.
	 */
	void proceed() {
	...
	/**
	 * Launches the TextMessagesComposerApp activity and passes the recipient
	 * name, number and type of number.
	 * 
	 * @param name
	 *            The name of the recipient.
	 * @param type
	 *            The type of the number to which the message is to be sent.
	 *            E.g. Mobile, Phone etc.
	 * @param number
	 *            The number of the recipient to which the message is to be
	 *            sent.
	 */
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		namesListView.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View view,
				...
				if (contactId.size() == 0) {
					startNewActivity(name.get(position).toString(),
							type.get(position).toString(), number.get(position)
									.toString());
				} else {
					startNewActivity(name.get(position).toString(),
							type.get(position).toString(), number.get(position)
		...
		namesListView.setOnKeyListener(new OnKeyListener() {
			...
			public boolean onKey(View view, int keyCode, KeyEvent keyEvent) {
				...
				if (keyEvent.getAction() == KeyEvent.ACTION_DOWN) {
					...
					case KeyEvent.KEYCODE_ENTER:
						if (contactId.size() == 0) {
							startNewActivity(name.get(currentSelection)
									.toString(), type.get(currentSelection)
									.toString(), number.get(currentSelection)
									.toString());
						} else {
							startNewActivity(name.get(currentSelection)
									.toString(), type.get(currentSelection)
					...
					case KeyEvent.KEYCODE_DPAD_DOWN:
						...
						if (currentSelection == lstView.getCount()) {
							...
							Utils.giveFeedback(
									...
									namesListView.getItemAtPosition(
		...
		editRecipient.addTextChangedListener(new TextWatcher() {
			...
			public void onTextChanged(CharSequence cs, int arg1, int arg2,
						...
						if (!Utils
							...
							if (cs.toString()
								...
								if (editRecipient.getText().toString()
										.matches("-?\\d+(\\.\\d+)?")) {
									TTS.readNumber(editRecipient.getText()
											.toString());
								} else {
									TTS.speak(editRecipient.getText()
											.toString());
								}
							} else {
				...
				if (!(editRecipient.getText().toString().trim().equals(""))
					...
					HashMap<String, ArrayList<String>> listOfNames = new ContactManager(
							getApplicationContext())
							.getNamesStartingWith(editRecipient.getText()
									.toString());
					...
					for (int i = 0; i < ((ArrayList<String>) (listOfNames
							.get("name"))).size(); i++) {
						if (!contactId
								.contains(((ArrayList<String>) (listOfNames
										.get("id"))).get(i).toString())) {
							name.add(((ArrayList<String>) (listOfNames
									.get("name"))).get(i).toString());
							contactId.add(((ArrayList<String>) (listOfNames
									.get("id"))).get(i).toString());
							type.add(((ArrayList<String>) (listOfNames
									.get("type"))).get(i).toString());
							number.add(((ArrayList<String>) (listOfNames
									.get("number"))).get(i).toString());
					...
					if (!(editRecipient.getText().toString().trim().equals(""))) {
						...
						HashMap<String, ArrayList<String>> listOfNames = new ContactManager(
								getApplicationContext())
								.getNamesWithNumber(editRecipient.getText()
										.toString());
						name = new ArrayList<String>();
						number = new ArrayList<String>();
						type = new ArrayList<String>();
