...
/** Activity that is used to write a text message and send to the selected recipient. **/
...
public class TextMessagesComposerApp extends Activity implements KeyListener{
	...
	/**
	* Attaches onKey listener to the button passed as a parameter to the method. 
	* If enter key on the keyboard or center key on the keypad is pressed, 
	* sendMessage method is called, which will be used to send the text message to the recipient.
	* @param button The button with which the onKey listener is to be associated.
	*/
					...
					if(editMessage.getText().toString().length() != 0) {
			        	...
			        	if(!Utils.isAccessibilityEnabled(getApplicationContext()) && 
			        		...
			        		if(editMessage.getText().toString().
			        				substring(editMessage.getText().toString().length()-1, 
			        						editMessage.getText().toString().length()).
        							matches("-?\\d+(\\.\\d+)?")) {
			        			TTS.speak("Deleted " + editMessage.getText().toString().
			        					substring(editMessage.getText().toString().length()-1, 
			        							editMessage.getText().toString().length()) + 
			        							". " + TTS.readNumber(editMessage.getText().toString().
					        					substring(0, 
					        							editMessage.getText().toString().length()-1)));
			        		}
			        		else {
			        			TTS.speak("Deleted " + editMessage.getText().toString().
			        					substring(editMessage.getText().toString().length()-1, 
			        	...
			        	editMessage.setText(editMessage.getText().toString().substring(0, 
			        			editMessage.getText().toString().length()-1));
			        	editMessage.setSelection(editMessage.getText().toString().length(), 
			        			editMessage.getText().toString().length());
						return false;
					}
					else {
	...
	protected void onCreate(Bundle savedInstanceState) {
       				...
       				//check if keyboard is connected but accessibility services are disabled
     		        	if(!Utils.isAccessibilityEnabled(getApplicationContext()) && 
       					 ...
       					 Toast.makeText(getApplicationContext(), getResources().
       							 getString(R.string.smsNotDelivered), Toast.LENGTH_SHORT).show();
       					 break;
   				 }
			 	}
			 };
	    ...
	    if(getIntent().hasExtra("name")) {
	    	...
	    	txtRecipient.setContentDescription(this.name.replaceAll(".(?=[0-9])", "$0 ") + " " + this.type);
	    }
	    	...
	    	if(txtRecipient.getText().toString().trim().equals(""))
	    		...
	    		txtRecipient.setContentDescription(this.number.replaceAll(".(?=[0-9])", "$0 "));
	    }
	            	...
	            	if(cs.length() > 0) {
			        	...
			        	if(!Utils.isAccessibilityEnabled(getApplicationContext()) && 
			        		...
			        		if(cs.toString().substring(cs.length()-1, cs.length()).matches("(?![@',&] )\\p{Punct}")) {
			        			if(editMessage.getText().toString().matches("-?\\d+(\\.\\d+)?")) {
			        				TTS.readNumber(editMessage.getText().toString());
			        			}
			        			else {
	...
	/**
	* Sends the text message to the the recipient. Informs the user about the status, whether the 
	...
	*/
	void sendMessage() {
		...
		Toast.makeText(getApplicationContext(), getResources().getString(R.string.sendingSms),
				Toast.LENGTH_SHORT).show();
	try {
