	...
	*/
	...
	public View getView(int position, View view, ViewGroup parent)
	{
		...
		if(this.callLogActivity == CALL_LOG) {
			int secondOccurrenceOfNewLineCharacter = values.get(position).indexOf("\n", 
					values.get(position).indexOf("\n")+1);
			textView.setContentDescription(values.get(position).substring(0, values.get(position).
					indexOf("\n")) + "\n" + values.get(position).substring(values.get(position).
					indexOf("\n") + 1, secondOccurrenceOfNewLineCharacter).
					replaceAll(".(?=[0-9])", "$0 ") + values.get(position).
					substring(secondOccurrenceOfNewLineCharacter).replaceAll(".(?=[:])", "$0 "));
		}
			...
			if(values.get(position).contains(":")) {android.util.Log.e("here","here");
				textView.setContentDescription(values.get(position).replaceAll(".(?=[:])", "$0 ")
				.replaceAll(".(?=[0-9])", "$0 "));
			}
			else {
