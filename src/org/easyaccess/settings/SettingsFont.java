...
public class SettingsFont extends EasyAccessActivity {
	...
	*/
	...
	/**
 	* Attaches onClick listener to the Button passed as a parameter to the method. The method 
 	* corresponding to the button being clicked is called.
 	* @param button This is an instance of Button.
 	*/
	...
	/**
 	* Attaches onKey listener to the Button passed as a parameter to the method. The method 
 	* corresponding to the button on which the enter key of the keyboard or the center key of the 
 	* keypad is pressed.
 	* @param button This is an instance of Button.
 	*/
	...
	/**
 	* Attaches onFocusChange listener to the TextView passed as parameter to the method.
 	* If a keyboard is connected to the device, a TTS feedback would be given to the user 
 	* informing him/her about the text on the TextView that received focus.
	...
	/**
 	* Attaches onItemSelected listener and onKey listener to the Spinner passed as a parameter to 
 	* the method. If an item is selected from the spinner, the selected font type is saved to 
 	* SharedPreferences. KeyListener is used to shift the focus in the spinner items using the 
 	* up and down arrow keys on the keyboard or the keypad.
 	* @param spinner This is an instance of Spinner.
 	*/
	void attachListenerToSpinnerItem(final Spinner spinner) {
		...
		spinner.setOnItemSelectedListener(new OnItemSelectedListener() {
			...
			public void onItemSelected(AdapterView<?> arg0, View view, int position,
					long arg3) {
					//flag value is used so that the font type is not applied when the activity is 
				//started. OnItemSelected is called after the activity is created.
					if(flag == 1) {
						...
						if(editor.commit()) {
							...
							if(!Utils.isAccessibilityEnabled(getApplicationContext()) && 
								...
								TTS.speak(spinner.getSelectedItem().toString() + " " + 
									getResources().getString(R.string.appliedTypeSuccess));
							...
							if(!Utils.isAccessibilityEnabled(getApplicationContext()) &&
									getResources().getConfiguration().keyboard != Configuration.KEYBOARD_NOKEYS)
								TTS.speak(spinner.getSelectedItem().toString() + " " + 
									getResources().getString(R.string.appliedTypeFailure));
							Toast.makeText(getApplicationContext(), spinner.getSelectedItem().toString() 
									+ " " + getResources().getString(R.string.appliedTypeFailure), 
									Toast.LENGTH_SHORT).show();
						}
				...
				if(keyEvent.getAction() == KeyEvent.ACTION_DOWN) {
					...
					case KeyEvent.KEYCODE_ENTER:
						if(currentSelection != -1) {
							String typeface = Integer.toString(currentSelection);
							//save in SharedPreferences
							preferences = getSharedPreferences(getResources().getString(R.string.fonttype), 0);
							if(typeface.equals(getResources().getString(R.string.none))) {
								txtPreview.setTypeface(Typeface.create("default", Typeface.NORMAL));
							...
							if(editor.commit()) {
					        	...
					        	if(!Utils.isAccessibilityEnabled(getApplicationContext()) && 
					        			getResources().getConfiguration().keyboard != Configuration.KEYBOARD_NOKEYS)
									TTS.speak(lstView.getSelectedItem().toString() + " " +
					        			getResources().getString(R.string.appliedTypeSuccess));
								Toast.makeText(getApplicationContext(), lstView.getSelectedItem().toString() 
										+ " " + getResources().getString(R.string.appliedTypeSuccess), 
										Toast.LENGTH_SHORT).show();
							}
							else {
					...
					case KeyEvent.KEYCODE_DPAD_DOWN:
						...
						if(currentSelection == lstView.getCount()) {
							...
							giveFeedback(spinner.getItemAtPosition(currentSelection).toString());
					...
					case KeyEvent.KEYCODE_DPAD_UP:
						...
						if(currentSelection <= -1) {
							...
							giveFeedback(lstView.getItemAtPosition(currentSelection).toString());
							lstView.setSelection(currentSelection);
						}
						break;
					}
	...
	void decrease() {
		...
		if(number - 1 >= Integer.valueOf(getResources().getString(R.string.lowerLimit))) {
			...
			if(editor.commit()) {
	        	...
	        	if(!Utils.isAccessibilityEnabled(getApplicationContext()) && 
					...
					TTS.speak(txtNumber.getText().toString() + " " + 
	        			getResources().getString(R.string.appliedFontSizeSuccess));
				Toast.makeText(getApplicationContext(), txtNumber.getText().toString() + " " + 
	        			getResources().getString(R.string.appliedFontSizeSuccess), Toast.LENGTH_SHORT).show();
			}
			else {
				//check if keyboard is connected and accessibility services are disabled
	        	if(!Utils.isAccessibilityEnabled(getApplicationContext()) && 
	        			getResources().getConfiguration().keyboard != Configuration.KEYBOARD_NOKEYS)
					TTS.speak(txtNumber.getText().toString() + " " + 
	        			getResources().getString(R.string.appliedFontSizeFailure));
				Toast.makeText(getApplicationContext(), txtNumber.getText().toString() + " " + 
	        			getResources().getString(R.string.appliedFontSizeFailure), Toast.LENGTH_SHORT).show();
			}
		}
		else {
			//check if keyboard is connected and accessibility services are disabled
			...
			Toast.makeText(getApplicationContext(), getResources().getString(R.string.reachedLowerLimit), 
					Toast.LENGTH_SHORT).show();
		}
	}
	...
	void increase() {
		...
		if(number + 1 <= Integer.valueOf(getResources().getString(R.string.maxLimit))) {
			...
			Toast.makeText(getApplicationContext(), getResources().getString(R.string.reachedMaxLimit),
					Toast.LENGTH_SHORT).show();
		}
	}
	
	/**
 	* Clears the font size and type values from SharedPreferences and resets the font size and type
 	* used across the app to their default values.
 	*/	
	void reset() {
		...
		txtNumber.setText(Float.toString(getSharedPreferences(getResources().getString(R.string.size), 0).
				getFloat("size", Float.valueOf(getResources().getString(R.string.defaultFontSize)))));
		txtNumber.setContentDescription(Float.toString(getSharedPreferences(getResources().getString(R.string.size), 0).
				getFloat("size", Float.valueOf(getResources().getString(R.string.defaultFontSize)))));
		LinearLayout layout = (LinearLayout) findViewById(R.id.settingsfont);
		Utils.applyFontTypeChanges(getApplicationContext(), layout);
		Utils.applyFontSizeChanges(getApplicationContext(), layout);
		...
		Toast.makeText(getApplicationContext(), getResources().getString(R.string.resetFontSuccess), 
				Toast.LENGTH_SHORT).show();
	}
	
	/**
 	* Announces the text that is passed as a parameter and causes the device to vibrate for 300 
 	* milliseconds.
 	* @param text This is the string that is to be read aloud.
 	*/
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		/** Attach OnKey listener to increase button, decrease button and reset button 
	...
	public void onResume() {
		...
		/** Display the selected text size and text type in the TextView and the Spinner, respectively **/
		txtNumber.setText(Float.toString(getSharedPreferences(getResources().getString(R.string.size), 0).
				getFloat("size", Float.valueOf(getResources().getString(R.string.defaultFontSize)))));
		txtNumber.setContentDescription(Float.toString(getSharedPreferences(getResources().getString(R.string.size), 0).
				getFloat("size", Float.valueOf(getResources().getString(R.string.defaultFontSize)))));
		spinnerFontType.setSelection(getSharedPreferences(getResources().
				getString(R.string.fonttype), 0).getInt("typeface", 0));
    	...
    	if(Utils.isAccessibilityEnabled(getApplicationContext()) ||
    		...
    		new java.util.Timer().schedule( 
    		        new java.util.TimerTask() {
    		            @Override
    		            public void run() {
    		            	TTS.speak("Font Size " + Float.toString(getSharedPreferences(
    		            			getResources().getString(R.string.size), 0).
    		        				getFloat("size", Float.valueOf(getResources().
    		        				getString(R.string.defaultFontSize)))) + ", Font Type "
   		        					+ Utils.fontType.get(getSharedPreferences(getResources().
    		        								getString(R.string.fonttype), 0).getInt("typeface", 0)));
    		            }
    		        }, 
    		        1000 
