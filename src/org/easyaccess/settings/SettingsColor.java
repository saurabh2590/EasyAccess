...
/** SettingsColor allows the user to select the text color and background color for all the screens
 *  in the app **/
public class SettingsColor extends EasyAccessActivity {
	...
	*/
 	...
 	* @param position This is the position of the spinner item that was selected by the user.
 	*/
	...
	void applyColor(Spinner spinner, int position) {
		...
		case R.id.fcolors: //spinner associated with text colors was passed as parameter
			...
			if(editor.commit()) {
	        	...
	        	if(!Utils.isAccessibilityEnabled(getApplicationContext()) && 
					...
					TTS.speak(spinner.getSelectedItem().toString() + " " + 
	        			getResources().getString(R.string.appliedFgColorSuccess));
				Toast.makeText(getApplicationContext(), spinner.getSelectedItem().toString() + " " 
	        			+ getResources().getString(R.string.appliedFgColorSuccess), Toast.LENGTH_SHORT).show();
			}
			else {
				//check if keyboard is connected and accessibility services are disabled
	        	if(!Utils.isAccessibilityEnabled(getApplicationContext()) &&
	        			getResources().getConfiguration().keyboard != Configuration.KEYBOARD_NOKEYS)
					TTS.speak(spinner.getSelectedItem().toString() + " " + 
	        			getResources().getString(R.string.appliedFgColorFailure));
				Toast.makeText(getApplicationContext(), spinner.getSelectedItem().toString() + " " 
	        			+ getResources().getString(R.string.appliedFgColorFailure), Toast.LENGTH_SHORT).show();
			}
			break;
		case R.id.bcolors: //spinner associated with background colors was passed as parameter
			//save in SharedPreferences
			preferences = getSharedPreferences(getResources().getString(R.string.color), 0);
			editor = preferences.edit();
			...
			if(editor.commit()) {
	        	...
	        	if(!Utils.isAccessibilityEnabled(getApplicationContext()) && 
	        			getResources().getConfiguration().keyboard != Configuration.KEYBOARD_NOKEYS)
					TTS.speak(spinner.getSelectedItem().toString() + " " + 
	        			getResources().getString(R.string.appliedBgColorSuccess));
				Toast.makeText(getApplicationContext(), spinner.getSelectedItem().toString() + " "
	        			+ getResources().getString(R.string.appliedBgColorSuccess), Toast.LENGTH_SHORT).show();
			}
			else {
				//check if keyboard is connected and accessibility services are disabled
	        	if(!Utils.isAccessibilityEnabled(getApplicationContext()) && 
	        			getResources().getConfiguration().keyboard != Configuration.KEYBOARD_NOKEYS)
					TTS.speak(spinner.getSelectedItem().toString() + " " + 
	        			getResources().getString(R.string.appliedBgColorFailure));
				Toast.makeText(getApplicationContext(), spinner.getSelectedItem().toString() + " " 
	        			+ getResources().getString(R.string.appliedBgColorFailure), Toast.LENGTH_SHORT).show();
			}
			break;
		}
	...
	/**
 	* Attaches onKey listener to the Button passed as a parameter to the method. If enter key on 
 	* the keyboard or center key on the keypad is pressed, it resets the text color and 
 	* the background color of the app to their default values.
	...
	/**
 	* Clears the text color and background color from SharedPreferences and resets the text color
 	* and the background color of the app to their default values.
 	*/	
	void reset() {
		...
		Toast.makeText(getApplicationContext(), getResources().getString(R.string.resetColorsSuccess), 
				Toast.LENGTH_SHORT).show();
	}
	
	/**
 	* Attaches onItemSelected listener to the Spinner passed as a parameter to the method.
 	* @param spinner This is an instance of Spinner. The selected color is applied when an item of the
 	* spinner is selected.
 	*/
	void attachListenerToSpinnerItem(final Spinner spinner) {
		...
		spinner.setOnItemSelectedListener(new OnItemSelectedListener() {
			...
			public void onItemSelected(AdapterView<?> arg0, View view, int position,
					long arg3) {
				//flag value is used so that the color is not applied when the activity is started.
	...
	/** 
	* Retrieves the position of the color in the spinner based on its value in color.xml.
	* @param colorValue. This is the  integer value of the color specified in color.xml.
	**/
	...
	public void onResume() {
		...
		int fgColorValue = getSharedPreferences(getResources().getString(R.string.color), 0).getInt("fgcolor", -1);
		int bgColorValue = getSharedPreferences(getResources().getString(R.string.color), 0).getInt("bgcolor", -1);
    	...
    	if(Utils.isAccessibilityEnabled(getApplicationContext()) ||
    		...
    		if(fgColorValue == -1) {
    			...
    			msg += " Foreground Color " + Utils.colorNames.get(getColorIndex(fgColorValue));
    		}
    		if(bgColorValue == -1) {
    			msg += ",  Background Color White";
    		}
    		else {
    			msg += " Background Color " + Utils.colorNames.get(getColorIndex(bgColorValue));
    		}
